#!/usr/bin/env python3
"""
dethloff_affine_certified_alns_diag.py
...
original benchmark script (neither imported nor modified). Added on top:
  ...
  * a route-level geometry probe (chi, kappa, zeta, R/delta, nu, D_align, and
  ...
  * spectral accumulators A = sum K_r D_r D_r^T and M = sum b_t b_t^T reported
    ...
    Davis-Kahan quantities (Delta_A, eta=||E||/Delta_A, angle(u_SLOPE, v_A)),
...
This run is NOT for speed-up; it answers the four go/no-go questions:
  1. Does A_search have a dominant common direction (l2/l1 small)?
  2. Is eta = ||E||/Delta_A small enough for Davis-Kahan to bite?
  3. How close is global SLOPE to the delivery baseline / best-found direction in
     EXACT gamma?
  4. What is the distribution of the normalized margin (an identity-based diagnostic)?
"""
...
class SpectralAccumulator:
    ...
    def summary(self, u_slope: np.ndarray, label: str) -> dict:
        ...
        out["angle_uslope_vM_deg"] = angle(u_slope, vM)
...
class DiagContext:
    ...
    def spectral_summaries(self):
        return [
            self.spectral_call.summary(self.cache.u, "search_call_weighted"),
            self.spectral_unique_ordered.summary(self.cache.u, "search_unique_ordered_route"),
            self.A_unique_customer_set.summary(self.cache.u, "search_unique_customer_set_A_only"),
        ]
...
SPECTRAL_CSV_FIELDS = [
    ...
    "angle_uslope_vA_deg", "eigengap_degenerate",
]
...
def build_train_spectral(instance, cache, cfg):
    ...
    for route in routes:
        ...
        _, slopes = route_affine_lines(instance, route)
        acc.update(slopes)
    return acc.summary(cache.u, "train_random_unique")
    ...
    for file_index, file in enumerate(files, 1):
        ...
        for profile_name in config["profiles"]:
            ...
            try:
                ...
                if diagnostic:
                    train_spec = build_train_spectral(instance, cache, config)
                    ...
                    print(
                        ...
                        f"angle(u,vM)={_fmt(train_spec['angle_uslope_vM_deg'])}deg"
                    )
                    ...
                    if diagnostic:
                        ...
                        for spec in specs:
                            print(
                                ...
                                f"a(u,vM)={_fmt(spec['angle_uslope_vM_deg'])} "
                                f"a(u,vA)={_fmt(spec['angle_uslope_vA_deg'])}"
                            )
                    else:
                        row = benchmark_cell(
                            instance, profile_name, policy, capacity, config["alpha"], cache,
